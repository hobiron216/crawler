...
Path("/news_cn/requests/lowyat/request_new/").mkdir(parents=True,exist_ok=True)
...
for id in list_url_id:
    while looping:
        ...
        link = "https://forum.lowyat.net/topic/" + str(id) +"/+"  + str(i)
        print(i)
        print(link)
        while True:
            try:
        ...
        for div in divs:
            ...
            #     "comment": str(comment)
            # }
            list =[link, name, comment_time, converted_time, comment]
            # print(list)
            with open('/news_cn/requests/lowyat/request_new/lowyat_' + str(id) + '.csv', 'a',  newline='', encoding='utf-8') as f_object:
