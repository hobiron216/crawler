...
class BjNews:
    ...
    def __init__(self):
        ...
        self.list_link = []
        self.path = "/home/news/bharian/"
        self.site_name = "bharian"

    def get_items(self):
        category="rencana"
        sub_categorys= ['minda pembaca', 'komentar', 'agama', 'sastera', 'kolumnis', 'lain-lain(rencana)']

        for sub_category in sub_categorys:
            if sub_category == 'minda pembaca':
                url = "https://www.bharian.com.my/api/topics/58707?page="
                self.get_link(category,sub_category, url)
            elif sub_category == 'komentar':
                url ="https://www.bharian.com.my/api/topics/8477?page="
                self.get_link(category, sub_category, url)
            elif sub_category == 'agama':
                url = "https://www.bharian.com.my/api/topics/6?&page="
                self.get_link(category, sub_category, url)
            elif sub_category == 'sastera':
                url = "https://www.bharian.com.my/api/topics/8478?page="
                self.get_link(category, sub_category, url)
            elif sub_category == 'kolumnis':
                url = "https://www.bharian.com.my/api/topics/10?page="
                self.get_link(category, sub_category, url)
            elif sub_category == 'lain-lain(rencana)':
                url = "https://www.bharian.com.my/api/topics/146962?page="
                self.get_link(category, sub_category, url)
    ...
    def get_link(self, category, sub_category, url21):
        ...
        for page in range (0,100) :
            ...
            for data_json in r:
                ...
                content21 = str(content).replace("<p />","").replace("<p>","").replace("</p>","").replace("<strong />","").replace("<strong>","").replace("</strong>","").replace('\"','').replace("© New Straits Times Press (M) Bhd","").replace("</b>","").strip()
                content21 = content21.split("my")
                try :
                    content=content21[1].strip()
                    location = ""
                except :
                    continue
                try:
