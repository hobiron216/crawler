...
class Spider:
    ...
    def __init__(self):
        self.base_link = "https://www.malaysialistings.com/"
        self.website = "malaysialisting"
        self.a = 0
    ...
    def get_items(self):
        ...
        # with open("list_url.json", 'r', encoding='utf_8_sig') as outfile:
        #     self.list_url = json.load(outfile)

        # url_pages = ["listings", "featured"]
        # for url_page in url_pages:
        #     self.get_url_page(url_page)

        url = "https://www.malaysialistings.com/categories"
        ...
        print("crawl category")
        divs = soup.find("ul", class_="maincat-list").findAll("li")
        ...
        for div in divs:
            ...
            if "JPG" in href or "PNG" in href:
                continue
            # print(href)
            if "malaysialistings.com" in href:
                last_string_url = href[-1]
                ...
                self.list_category.append(nama_file)
    ...
    def get_url (self, url):
        # if url not in self.list_link_check:
        #     self.list_link_check.append(url)
        #     self.list_link.append(url)
        ...
        # print("save list url")
        # if self.list_link:
        #     with open("list_url.json", 'w', encoding='utf_8_sig') as outfile:
        #         json.dump(self.list_link, outfile, ensure_ascii=False)
        self.url_detail()

    def url_detail(self):

        for url in self.list_link:
            if url =="https://www.malaysialistings.com/":
                continue

            if url in self.list_link_check:
                continue

            if "return_uri=" in url:
                continue
            ...
            if nama_file == "listings":
                category = url.replace("https://www.malaysialistings.com/", "")
            else:
                category = url.replace("https://www.malaysialistings.com/","").replace(nama_file,"")
            ...
            print("save html")
            if nama_file in self.list_category:
                category = category + "/" + nama_file + "/page/"
                self.create_folder(category)
                with open(self.path + "/" + category + "1.html", 'w', encoding='utf8') as outfile:
                    outfile.write(str(soup))
            elif nama_file == "listings" or nama_file == "featured" or nama_file in self.list_category:
            ...
            for div in divs:
                ...
                if "https://www.malaysialistings.com/" in href:
                    if href not in self.list_link_check:
    ...
    def get_url_loop(self, list_url, nama_file21):
        ...
        while True:
            for url in list_url:
                ...
                if nama_file == "listings" or nama_file =="malaysia":
                    category = url.replace("https://www.malaysialistings.com/", "")
                else:
                    category = url.replace("https://www.malaysialistings.com/", "").replace(nama_file, "")
