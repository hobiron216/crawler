            ...
            if z == 2:
                ...
                if nama_pdf:
                    ...
                    try :
                        ...
                        path = category + "/search/pdf/" + category_tabel + "/" + company.strip() + "/" + registration_no + "/" + str(nama_pdf)
                        with open(path, 'wb') as out_file:
                            out_file.write(response.content)
                            out_file.close()
...
a=0
# companys = ['Medicell Pharmaceutical (Malaysia) Sdn. Bhd.', 'Mundipharma Pharmaceuticals Sdn. Bhd.']
for company in companys:
    ...
    if "." in company:
        ...
        company = company[0]
    if "Cambert" in company :
        company=company.strip()
    if "'" in company:
        company = company.split("'")
        company = company[0]
    if "Products" in company:
        company = company.split("Products")
        company = company[0].strip()
    print(company)
    data21 = {
    'func': 'search',
