...
class News_Cn:
    ...
    def __init__(self):
        ...
        self.a = 0
        self.random_sleep= [1,2,3,4,5,6,7,8,9]
    ...
    def get_items(self):
        ...
        for category in categorys:
            if category == 'politic' :
                ...
                # continue
                category_link = "politics/xhll"
                self.get_api_v1(category, category_link)
            elif category == 'opinion' :
                # continue
                category_link = "comments"
                self.get_api_v1(category,category_link)
            elif category == 'law' :
                # continue
                # nid="11227928"
    ...
    def get_api_v1(self,category,category_link):
        ...
        url= self.base_link + category_link
        print(url)
        while True:
            try:
        ...
        try :
            json_id = soup.find("div", id = "recommendDepth").attrs.get("data")
        except:
            json_id = soup.find("div", id="content-list").attrs.get("data")
        json_id = "ds_" + json_id.replace("datasource:","") + ".json"
        ...
        today = date.today()
        crawl_date21 = today.strftime("%Y-%m-%d")
        crawl_date21 = crawl_date21.split("-")
        year = crawl_date21[0]
        month = crawl_date21[1]
        date21 = crawl_date21[2]
        ...
        # html = json.load(html)
        datas=html['datasource']


        for data in datas:
            link= data['publishUrl']
            if year + "-" + month + "/" + date21 not in link:
                continue
    ...
    def parser(self,list_link, crawl_date, category):
        ...
        for url in list_link:
            if year + "-" + month + "/" + date21 not in url:
                continue
            url=str(url)
            print("url : " + url)
            b = True
