...
class Guangming:
    ...
    def create_folder(self, crawl_date, category):
        Path("/dataph/requests/news/guangming/batch/html/" + crawl_date + "/" + category + "/").mkdir(parents=True,exist_ok=True)
        Path("/dataph/requests/news/guangming/batch/json/" + crawl_date + "/" + category + "/").mkdir(parents=True,exist_ok=True)
    ...
    def get_items(self):
        ...
        for category in categorys:
            if category == 'politics' :
                nodes =['node_9844', 'node_9840', 'node_103380', 'node_9831', 'node_9828']
                for node in nodes:
    ...
    def parser(self,list_link, crawl_date, category, node):
        for url in list_link:
            ...
            path_html = "/dataph/requests/news/guangming/batch/html/" + crawl_date + "/" + category + "/" + href21 + ".html"
            ...
            self.a = self.a + 1
            # print (result)
            with open("/dataph/requests/news/guangming/batch/html/" + crawl_date + "/" + category + "/" + href21 + ".html",'w',encoding='utf8') as outfile:
                outfile.write(str(soup))
            with open("/dataph/requests/news/guangming/batch/json/" + crawl_date + "/" + category + "/" + href21 + ".json",'w',encoding='utf8') as outfile:
