...
class Guangming:
    ...
    def get_items(self):
        with open(self.path + "/cek_url1.json", 'r', encoding='utf_8_sig') as outfile:
        ...
        for category in categorys:
            ...
            if category == 'politics' :
                ...
                for node in nodes:
                    self.z = 0
                    self.list_url2 = []
                    with open(self.path + "/cek_url_" + category + "_" + node + ".json", 'r', encoding='utf_8_sig') as outfile:
                        self.list_url = json.load(outfile)
                    # with open(self.path + "/cek_url_" + category + "_" + node + ".json", 'w',
                ...
                for node in nodes:
                    ...
                    with open(self.path + "/cek_url_" + category + "_" + node + ".json", 'r', encoding='utf_8_sig') as outfile:
                        self.list_url = json.load(outfile)
                    # with open(self.path + "/cek_url_" + category + "_" + node + ".json", 'w', encoding='utf_8_sig') as outfile:
                    #     json.dump(self.list_url2, outfile, ensure_ascii=False)
                    # continue
                    self.get_html(category,node)
